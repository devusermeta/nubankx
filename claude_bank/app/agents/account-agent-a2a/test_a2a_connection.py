"""
Test A2A connection to verify AccountAgent in Foundry is being used

This script sends a test message to the A2A service and verifies:
1. The service responds successfully
2. The response comes from the AccountAgent in Azure AI Foundry
"""
...
async def test_a2a_connection():
    """Test that A2A service connects to Foundry AccountAgent"""
    
    print("=" * 70)
    print("  Testing A2A Connection to Azure AI Foundry AccountAgent")
    print("=" * 70)
    print()
    ...
    test_message = {
        "messages": [
            {
                "role": "user",
                "content": "Hello, are you the AccountAgent from Azure AI Foundry?"
            }
        ],
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        try:
            ...
            if "messages" in result and len(result["messages"]) > 0:
                ...
                print("=" * 70)
                print()
                print("The AccountAgent in Azure AI Foundry is responding correctly.")
