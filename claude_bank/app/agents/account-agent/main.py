"""
Account Agent Service - A2A enabled microservice for account operations.
...
from a2a_handler import AccountAgentHandler
...
# Global state
handler: AccountAgentHandler = None
...
async def lifespan(app: FastAPI):
    ...
    # Initialize handler
    handler = AccountAgentHandler(config)
...
# Create FastAPI app
app = FastAPI(
    title="Account Agent Service",
    description="A2A-enabled microservice for account operations",
    version=config.AGENT_VERSION,
    lifespan=lifespan,
)
...
async def a2a_invoke(message: A2AMessage, request: Request) -> A2AResponse:
    ...
    try:
        ...
        response = A2AResponse(
            ...
            source={
                "agent_id": config.AGENT_ID or "account-agent-001",
                "agent_name": config.AGENT_NAME,
            },
