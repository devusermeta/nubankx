"""
AIMoneyCoach Agent Service - A2A enabled microservice for financial coaching.

This agent handles:
- Debt management coaching
- Financial health assessment
- Clarification-first approach for personalized advice
"""
import os
import sys
import asyncio
...
from a2a_handler import AIMoneyCoachAgentHandler

config = AgentConfig()
logger = get_logger(__name__)

handler: AIMoneyCoachAgentHandler = None
...
async def lifespan(app: FastAPI):
    ...
    handler = AIMoneyCoachAgentHandler(config)
    ...
    if config.AGENT_REGISTRY_URL:
        ...
        try:
            agent_id = await registry_client.register(
                ...
                capabilities=[
                    "coaching.debt_management",
                    "coaching.financial_health",
                ...
                metadata={
                    "description": "Provides financial coaching and health assessment",
                    "mcp_tools": ["AIMoneyCoach.search", "AIMoneyCoach.createTicket"],
                    "output_formats": ["COACHING_CARD", "HEALTH_ASSESSMENT"],
                },
            )
...
app = FastAPI(
    title="AIMoneyCoach Agent Service",
    description="A2A-enabled microservice for financial coaching",
    version=config.AGENT_VERSION,
    lifespan=lifespan,
)
