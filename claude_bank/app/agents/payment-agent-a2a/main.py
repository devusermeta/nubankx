"""
Payment Agent A2A Microservice - FastAPI Server
...
from agent_handler import get_payment_agent_handler, cleanup_handler
from config import (
    A2A_SERVER_PORT,
    A2A_SERVER_HOST,
    PAYMENT_AGENT_NAME,
...
AGENT_CARD = {
    "name": "Payment Agent",
    "description": (
        "Specialized banking agent for payment operations. "
        "Handles payment initiation, beneficiary management, payment history, "
        "and payment status tracking. Uses MCP tools to access payment systems, "
        "account data, transaction history, and contact management."
    ),
    "url": f"http://localhost:{A2A_SERVER_PORT}",
    "version": "1.0.0",
    "capabilities": ["payment_initiation", "beneficiary_management", "payment_history", "payment_status"],
    "agent_id": f"{PAYMENT_AGENT_NAME}:{PAYMENT_AGENT_VERSION}",  # NEW format: name:version
    "endpoints": {
    ...
    "metadata": {
        ...
        "mcp_servers": ["account", "transaction", "payment", "contacts"],
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 Starting Payment Agent A2A Microservice...")
    ...
    logger.info("✅ Payment Agent Handler initialized")
    
    logger.info(f"✅ Payment Agent A2A Server ready on {A2A_SERVER_HOST}:{A2A_SERVER_PORT}")
    ...
    # Cleanup on shutdown
    logger.info("🛑 Shutting down Payment Agent A2A Microservice...")
...
# Create FastAPI app
app = FastAPI(
    title="Payment Agent A2A Server",
    description="Banking payment specialist agent exposed via A2A protocol",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def root():
    ...
    return JSONResponse(
        content={
            "service": "Payment Agent A2A Microservice",
            "version": "1.0.0",
...
if __name__ == "__main__":
    logger.info("Starting Payment Agent A2A server...")
    uvicorn.run(
        "main:app",
