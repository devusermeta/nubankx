"""
Create TransactionAgent in Azure AI Foundry with A2A support

This script creates the TransactionAgent in Azure AI Foundry (cloud service) and configures it
...
Run this ONCE to create the agent in Foundry:
    uv run python create_transaction_agent_in_foundry.py
"""
...
async def create_transaction_agent():
    """Create TransactionAgent in Azure AI Foundry"""
    ...
    if instructions_file.exists():
        ...
        instructions = """You are TransactionAgent, a specialized banking assistant for transaction history.
...
- Account data (balances, account information)
- Transaction history

Always provide clear transaction information and use HTML tables for multiple transactions.
Follow banking security best practices."""
    ...
    async with (
        ...
        print(f"   TRANSACTION_AGENT_NAME={azure_agent.name}")
        print(f"   TRANSACTION_AGENT_VERSION={azure_agent.version}")
        print("")
        ...
        print("   Set USE_A2A_FOR_TRANSACTION_AGENT=true in claude_bank/.env")
