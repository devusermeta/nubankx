"""
Test A2A connection to verify TransactionAgent in Foundry is being used

This script sends a test message to the A2A service and verifies:
1. The service responds successfully
2. The response comes from the TransactionAgent in Azure AI Foundry
"""
...
async def test_a2a_connection():
    """Test that A2A service connects to Foundry TransactionAgent"""
    
    print("=" * 70)
    print("  Testing A2A Connection to Azure AI Foundry TransactionAgent")
    print("=" * 70)
    print()
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            response = await client.get("http://localhost:9002/.well-known/agent.json")
    ...
    test_message = {
        "messages": [
            {
                "role": "user",
                "content": "Hello, are you the TransactionAgent from Azure AI Foundry?"
            }
        ],
        "customer_id": "CUST001",
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        try:
            response = await client.post(
                "http://localhost:9002/a2a/invoke",
                json=test_message,
            ...
            if "messages" in result and len(result["messages"]) > 0:
                ...
                print("=" * 70)
                print()
                print("The TransactionAgent in Azure AI Foundry is responding correctly.")
